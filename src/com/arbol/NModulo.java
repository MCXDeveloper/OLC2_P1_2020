...
public class NModulo extends Nodo implements Instruccion {
            ...
            if (v1.getTipoDato() == ETipoDato.INT && v2.getTipoDato() == ETipoDato.INT) {
                ...
                */
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoVector)) {
                    msj = "Error. No hay implementación para la operación MODULO para los tipos <VECTOR["+ tipoInternoVector +"]> y <"+ v2.getTipoDato() +">.";
                    ...
                    for (Item i : v.getElementos()) {
                        ...
                        r = new NModulo(getLinea(), getColumna(), getArchivo(), op1, op2).Ejecutar(ts);
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoVector)) {
                    msj = "Error. No hay implementación para la operación MODULO para los tipos <"+ v1.getTipoDato() +"> y <VECTOR["+ tipoInternoVector +"]>.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                } else {
                    NPrim op2;
                    Resultado r;
                ...
                if (vec1.getVectorSize() == vec2.getVectorSize()) {
                    ...
                    msj = "Error. Los tamaños de los vectores difieren por lo que no se puede realizar el módulo.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                }
                ...
                */
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoMatriz)) {
                    msj = "Error. No hay implementación para la operación MODULO para los tipos <MATRIX["+ tipoInternoMatriz +"]> y <"+ v2.getTipoDato() +">.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                } else {
                    NPrim op1;
                    Resultado r;
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoMatriz)) {
                    msj = "Error. No hay implementación para la operación MODULO para los tipos <"+ v1.getTipoDato() +"> y <MATRIX["+ tipoInternoMatriz +"]>.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                } else {
                    NPrim op2;
                    Resultado r;
                ...
                if (v.getVectorSize() > 1) {
                    msj = "Error. No se puede realizar la módulo de un vector de más de 1 elemento con una matriz.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                } else {
                    Matriz mat = (Matriz)v2.getValor();
                    NPrim op2;
                ...
                Vector v = (Vector)v2.getValor();
                if (v.getVectorSize() > 1) {
                    msj = "Error. No se puede realizar la módulo de una matriz y un vector de más de 1 elemento.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                } else {
                    Matriz mat = (Matriz)v1.getValor();
                    NPrim op1;
                ...
                if (mat1.getSize() == mat2.getSize()) {
                    ...
                    msj = "Error. Las dimensiones de la matriz difieren por lo que no se puede realizar el módulo.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_MODULO]", msj, getLinea(), getColumna());
                }
    ...
    public String GenerarDOT(TablaSimbolos ts) {
        ...
        String subson = ts.getDeclararNodo("NODO_MODULO");
