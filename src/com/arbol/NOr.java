...
public class NOr extends Nodo implements Instruccion {

    private Nodo opIzq;
    private Nodo opDer;

    public NOr(int linea, int columna, String archivo, Nodo opIzq, Nodo opDer) {
        super(linea, columna, archivo, ETipoNodo.EXP_OR);
        this.opIzq = opIzq;
        this.opDer = opDer;
    }
            ...
            if (v1.getTipoDato() == ETipoDato.BOOLEAN && v2.getTipoDato() == ETipoDato.BOOLEAN) {
                ...
                */
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoVector)) {
                    msj = "Error. No hay implementación para la operación OR para los tipos <"+ v1.getTipoDato() +"> y <VECTOR["+ tipoInternoVector +"]>.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                } else {
                    NPrim op2;
                    Resultado r;
                    ...
                    for (Item i : v.getElementos()) {
                        ...
                        r = new NOr(getLinea(), getColumna(), getArchivo(), op1, op2).Ejecutar(ts);
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoVector)) {
                    msj = "Error. No hay implementación para la operación OR para los tipos <VECTOR["+ tipoInternoVector +"]> y <"+ v2.getTipoDato() +">.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                } else {
                    NPrim op1;
                    Resultado r;
                ...
                if (vec1.getVectorSize() == vec2.getVectorSize()) {
                    ...
                    msj = "Error. Los tamaños de los vectores difieren por lo que no se puede realizar la operación OR.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                }
                ...
                */
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoMatriz)) {
                    msj = "Error. No hay implementación para la operación OR para los tipos <"+ v1.getTipoDato() +"> y <MATRIX["+ tipoInternoMatriz +"]>.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                } else {
                    NPrim op2;
                    Resultado r;
                ...
                if (!Arrays.asList(tiposPermitidos).contains(tipoInternoMatriz)) {
                    msj = "Error. No hay implementación para la operación OR para los tipos <MATRIX["+ tipoInternoMatriz +"]> y <"+ v2.getTipoDato() +">.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                } else {
                    NPrim op1;
                    Resultado r;
                ...
                if (v.getVectorSize() > 1) {
                    msj = "Error. No se puede realizar la operación OR de un vector de más de 1 elemento con una matriz.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                } else {
                    Matriz mat = (Matriz)v2.getValor();
                    NPrim op2;
                ...
                Vector v = (Vector)v2.getValor();
                if (v.getVectorSize() > 1) {
                    msj = "Error. No se puede realizar la operación OR de una matriz y un vector de más de 1 elemento.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                } else {
                    Matriz mat = (Matriz)v1.getValor();
                    NPrim op1;
                ...
                if (mat1.getSize() == mat2.getSize()) {
                    ...
                    msj = "Error. Las dimensiones de la matriz difieren por lo que no se puede realizar la operación OR.";
                    ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
                }

            } else {
                msj = "Error. No hay implementación para la operación OR para los tipos <"+ v1.getTipoDato() +"> y <"+ v2.getTipoDato() +">.";
                ErrorHandler.AddError(getTipoError(), getArchivo(), "[N_OR]", msj, getLinea(), getColumna());
            }
    ...
    public String GenerarDOT(TablaSimbolos ts) {
        ...
        String subson = ts.getDeclararNodo("NODO_OR");
